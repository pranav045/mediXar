...
<head>
    ...
    <title>@yield('title', 'MedixAR | Master Human Anatomy with AR')</title>
...
<body class="bg-[#050b09] text-white min-h-screen flex flex-col selection:bg-teal-500/30 overflow-x-hidden">
    ...
    <nav class="fixed top-0 w-full z-50 glass-nav transition-all duration-300">
        ...
        <div id="mobile-menu" class="hidden absolute top-[72px] left-0 w-full glass-nav border-t-0 border-x-0 shadow-2xl flex flex-col p-6 gap-4 animate-in slide-in-from-top duration-300">
            ...
            @auth
                ...
                <a href="/profile" class="text-lg font-medium text-gray-300">My Profile</a>
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit" class="text-lg font-medium text-red-400">Logout</button>
                </form>
            @else
    ...
    <script>
        ...
        // Standard Scroll Behavior for Nav
        window.addEventListener('scroll', () => {
            const nav = document.querySelector('nav');
            if (window.scrollY > 20) {
                nav.classList.add('shadow-xl', 'bg-black/80');
                nav.classList.remove('bg-black/40');
            } else {
                nav.classList.remove('shadow-xl', 'bg-black/80');
            }
        });
    </script>
    @yield('scripts')
