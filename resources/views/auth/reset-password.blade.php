        ...
        <div class="w-full max-w-6xl max-h-[90vh]">
            ...
            <div class="glass-panel w-full h-full rounded-3xl flex flex-col lg:flex-row overflow-hidden shadow-2xl">
                    ...
                    <div class="mb-10 relative z-10">
                        <a href="/" class="flex items-center gap-3 group inline-flex">
                            <div class="w-16 h-16 relative flex items-center justify-center group-hover:scale-105 transition-transform duration-300">
                                ...
                                <div class="absolute inset-0 border-2 border-emerald-400/30 rounded-xl transform -rotate-12 group-hover:-rotate-90 transition-all duration-700"></div>
                                <svg class="w-10 h-10 text-teal-400 drop-shadow-[0_0_20px_rgba(45,212,191,0.6)] relative z-10" fill="currentColor" viewBox="0 0 512 512">
                ...
                <div class="lg:w-[35%] p-10 sm:p-12 flex flex-col justify-center bg-black/40">
                    ...
                    @if (session('status'))
                    <div class="alert-error flex items-start gap-3 px-4 py-3 rounded-2xl bg-teal-500/15 border border-teal-500/30 mb-5">
                        ...
                        <p class="text-teal-300 text-xs">{{ session('status') }}</p>
                    </div>
                    @endif

                    @if($errors->any())
                    <div class="alert-error flex items-start gap-3 px-4 py-3 rounded-2xl bg-red-500/15 border border-red-500/30 mb-5">
                    ...
                    <form method="POST" action="/reset-password" class="space-y-5">
                        ...
                        <div>
                            ...
                            <input type="email" name="email" value="{{ old('email', $email) }}" required placeholder="you@university.edu"
                                class="form-input w-full rounded-full px-5 py-3.5 text-sm {{ $errors->has('email') ? 'is-error' : '' }}" readonly>
                        </div>

                        <div>
                            <label class="text-xs text-gray-400 uppercase tracking-wider block mb-2">New Password</label>
                            <input type="password" name="password" required placeholder="Min 8 characters"
                                class="form-input w-full rounded-full px-5 py-3.5 text-sm {{ $errors->has('password') ? 'is-error' : '' }}" minlength="8">
                            @error('password')
                                <p class="field-error">
                        ...
                        <div>
                            ...
                            <input type="password" name="password_confirmation" required placeholder="Repeat new password"
                                class="form-input w-full rounded-full px-5 py-3.5 text-sm {{ $errors->has('password_confirmation') ? 'is-error' : '' }}" minlength="8">
