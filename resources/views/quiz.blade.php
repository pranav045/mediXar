...
<body class="bg-[#050b09] text-white min-h-screen flex flex-col selection:bg-teal-500/30 pb-20">
    ...
    <main class="flex-grow max-w-7xl w-full mx-auto px-6 pt-10 flex flex-col gap-10">
        ...
        <header class="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-white/10">
            <div>
                <h1 class="text-4xl md:text-5xl font-bold mb-2">Available <span class="text-teal-400">Quizzes</span></h1>
                <p class="text-gray-400 text-lg">Test your knowledge across different anatomical systems.</p>
            </div>
            @if(session('success'))
            <div class="px-6 py-3 rounded-xl bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">
                {{ session('success') }}
            </div>
            @endif
        </header>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="glass-card p-6 rounded-2xl flex flex-col h-full relative overflow-hidden group">
                <div class="absolute -right-10 -top-10 w-32 h-32 bg-teal-500/10 rounded-full blur-2xl group-hover:bg-teal-500/20 transition-all duration-500"></div>
                
                <div class="flex justify-between items-start mb-4">
                    <span class="px-3 py-1 rounded-full bg-white/5 border border-white/10 text-xs font-semibold text-gray-300">
                        {{ $quiz->category }}
                    </span>
                    @php
                        $colorClass = $quiz->difficulty === 'Easy' ? 'text-emerald-400' : ($quiz->difficulty === 'Medium' ? 'text-amber-400' : 'text-red-400');
                    @endphp
                    <span class="text-xs font-bold uppercase tracking-wider {{ $colorClass }}">
                        {{ $quiz->difficulty }}
                    </span>
                </div>
                
                <h3 class="text-2xl font-bold mb-2 relative z-10">{{ $quiz->title }}</h3>
                <p class="text-gray-400 text-sm mb-6 flex-grow relative z-10">{{ $quiz->description }}</p>
                
                <div class="pt-6 border-t border-white/10 flex justify-between items-center relative z-10">
                    <a href="{{ route('quiz.show', $quiz->id) }}" class="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-teal-500 text-black font-bold hover:bg-teal-400 transition w-full justify-center">
                        Start Quiz →
                    </a>
                </div>
            </div>
